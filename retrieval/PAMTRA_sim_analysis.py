...
class PAMTRASIM_analysis():
    def __init__(self,date="20110317",data_path="/scratch/u/u300737/",hour="12",instrument="hamp",
                height_idx=0,heights=[833000.,16000., 15400.,15200.,15000.,14800.,14600.,14400.,14200.,14000.,13800.,13600.,
    ...
    def get_era5_open_ocean_mask(self,):
        self.land_sea_mask=pd.Series(data=self.era5_ds["sfc_slf"].values.flatten(),
                                     index=range(self.era5_ds["sfc_slf"].shape[0]))
        self.sea_ice_mask=pd.Series(data=self.era5_ds["sfc_sif"].values.flatten(),
    ...
    def get_era5_hmps(self,only_ocean=False):
        ...
        iwv=pd.Series(data=self.era5_ds["iwv"][:,0,self.height_idx],index=self.sea_ice_mask.index)
        try:
            lwp=pd.Series(data=self.era5_ds["lwp"][:,0,self.height_idx],index=self.sea_ice_mask.index)
        except:
